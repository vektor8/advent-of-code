...
fn main() {
    ...
    let mut res = 0;
    for i in 0..bricks.len() {
        let supporters = supports[i]
            .iter()
            .filter(|&e| supported_by[*e].len() == 1)
            .copied()
            .collect::<Vec<usize>>();
        let mut q = VecDeque::from(supporters.clone());
        let mut falling: HashSet<usize> = HashSet::from_iter(supporters.iter().cloned());
        falling.insert(i);

        while let Some(current) = q.pop_front() {
            for k in &supports[current] {
                if falling.contains(k) {
                    continue;
                }
                if supported_by[*k].iter().all(|e| falling.contains(e)) {
                    q.push_back(*k);
                    falling.insert(*k);
                }
            }
