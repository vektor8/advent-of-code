...
fn find_longest_loop(start_pos: (usize, usize), grid: &Vec<Vec<char>>) -> (usize, Vec<Vec<usize>>) {
    ...
    let mut possible_lens: Vec<(usize, Vec<Vec<usize>>)> = vec![];
    while !queue.is_empty() {
        ...
        get_reachable_neighbours(pos, grid).iter().for_each(|&n| {
            if step_grid[n.0][n.1] == 0 {
                ...
                possible_lens.push((len, step_grid.clone()));
            }
        });
    }
    possible_lens.iter().max_by_key(|x| x.0).unwrap().to_owned()
}

fn map_start_to_pipe(start_pos: (usize, usize), grid: &Vec<Vec<char>>) -> char {
    let has_north = start_pos.0 > 0 && grid[start_pos.0 - 1][start_pos.1] != '.';
    let has_south = start_pos.0 < grid.len() - 1 && grid[start_pos.0 + 1][start_pos.1] != '.';
    let has_west = start_pos.1 > 0 && grid[start_pos.0][start_pos.1 - 1] != '.';
    let has_east = start_pos.1 < grid[0].len() - 1 && grid[start_pos.0][start_pos.1 + 1] != '.';

    match (has_north, has_south, has_east, has_west) {
        (_, true, true, _) => 'F',
        (true, true, _, _) => '|',
        (true, _, true, _) => 'L',
        (true, _, _, true) => 'J',
        (_, true, _, true) => '7',
        (_, _, true, true) => '-',
        _ => panic!("No valid pipe for start pos"),
    }
}
...
fn solve(input: &Vec<Vec<char>>) {
    ...
    let (_, grid) = find_longest_loop(start_pos, input);
    ...
    let mut grid: Vec<Vec<char>> = input
        ...
        .map(|(i, l)| {
            l.iter()
                .enumerate()
                .map(|(j, e)| if grid[i][j] == 0 { '.' } else { e.to_owned() })
                .collect()
        })
        .collect();

    grid[start_pos.0][start_pos.1] = map_start_to_pipe(start_pos, &grid);
    let mut count = 0;
